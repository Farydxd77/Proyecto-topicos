...
autenticado del grupo, registra un gasto con `descripcion`, `monto`, `pagadorId` y
`fecha`. `descripcion` MUST ser una cadena no vacía de hasta 255 caracteres;
`monto` MUST ser un número mayor que 0 con hasta 2 decimales; `fecha` MUST ser una
fecha válida (`YYYY-MM-DD`); `pagadorId` MUST identificar a un participante que sea
miembro del grupo. El sistema SHALL calcular la división del `monto` entre todos
los miembros actuales del grupo y SHALL responder `201 Created` con el gasto
creado y su división (`id`, `grupoId`, `descripcion`, `monto`, `pagador`, `fecha` y
la lista `division` de pares `participante` + `montoAdeudado`).
...
- **AND** responde `201 Created` con el gasto y su `division`
...
`fecha` del gasto indicado. Los cuatro campos son obligatorios y se validan igual
que en el registro. El sistema SHALL descartar la división anterior y SHALL
recalcular la división del nuevo `monto` entre **los miembros actuales del grupo**
al momento de la edición, aplicando la misma regla de redondeo. El sistema SHALL
responder `200 OK` con el gasto actualizado y su nueva `division`.
...
- **THEN** el sistema reemplaza las filas de `gasto_participantes` del gasto por la
  división del nuevo `monto` entre los miembros actuales
