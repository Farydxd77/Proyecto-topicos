...
The backend codebase SHALL be organized into a consistent set of packages (such as configuration, web/controller, service, repository, and entity layers) under the base package, so that future capabilities have a predictable place to be added.

#### Scenario: New capability follows existing structure
- **WHEN** a developer adds a new backend capability
- **THEN** its code is placed into the corresponding existing package (controller, service, repository, entity, etc.) rather than requiring a new top-level layout
